...
- `loading: boolean` : 위치 정보를 가져오는 중인지 여부
- `error: GeolocationPositionError | null` : 발생한 오류
- `timestamp?: EpochTimeStamp` : 위치 정보의 타임스탬프
- `latitude?: number` : 현재 위치의 위도
- `longitude?: number` : 현재 위치의 경도
- `altitude?: number` : 현재 위치의 고도
- `accuracy?: number` : 위치 정보의 정확도
- `altitudeAccuracy?: number` : 고도 정보의 정확도
- `heading?: number` : 이동 방향
- `speed?: number` : 현재 속도
